...
/**
 * Counts — get the site data counts section of the system-status payload.
 *
 * @since 1.0.14
 */
class Counts {
	...
	private function count_active_sessions(): int {
		try {
			return (int) $this->context->get_db()->get_var(
				...
				array(
					'active_since' => $this->get_session_cutoff(),
				),
			);
		} catch ( \Throwable $exception ) {
			unset( $exception );
	 ...
	 * @since 1.0.14
	 */
	private function get_session_cutoff(): string {
		return gmdate(
			'Y-m-d H:i:s',
			time() - max(
				0,
