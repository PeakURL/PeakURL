...
require_once $environment->get_runtime_root() . '/utils/string.php';
...
$autoload     = $environment->get_vendor_autoload_path();
...
$database_connection_error_path = $app_url( $base_path, '/database-error.php' );

if ( '/db-error.php' === $relative_path ) {
	header( 'Location: ' . $database_connection_error_path, true, 301 );
	exit();
}
...
if ( Str::starts_with( $relative_path, '/api/' ) ) {
	...
	require $environment->get_api_entrypoint();
	exit();
}
...
$dashboard_html_path = $environment->get_app_html_path();
