<?php
/**
 * Authentication and security endpoints controller.
 *
...
/**
 * Controller — Thin HTTP entry adapter delegating to Auth Service.
 *
 * @since 1.0.0
 */
class Controller extends BaseController {
	 ...
	 *
	 * @param Service      $auth_service    Auth domain service.
	 * @param Captcha|null $captcha_service Optional captcha service.
	 * @since 1.0.0
	 */
	public function __construct( Service $auth_service, ?Captcha $captcha_service = null ) {
	...
	public function verify_email( Request $request ): array {
		...
		$verified = $this->auth_service->verify_email( $token );
	...
	public function verify_two_factor_login( Request $request ): array {
		return $this->success_response(
			$this->auth_service->verify_two_factor_login(
				$request,
	...
	public function validate_reset_token( Request $request ): array {
		...
		if ( ! $this->auth_service->validate_reset_token( $token ) ) {
			return $this->not_found_response(
	...
	public function generate_api_key( Request $request ): array {
		...
		$key   = $this->auth_service->add_api_key( $request, $label );
	...
	public function delete_api_key( Request $request ): array {
		$deleted = $this->auth_service->delete_api_key(
			$request,
	...
	public function get_security( Request $request ): array {
		return $this->success_response(
			$this->auth_service->get_security_settings( $request ),
	...
	public function start_two_factor_setup( Request $request ): array {
		return $this->success_response(
			$this->auth_service->start_two_factor_setup( $request ),
	...
	public function disable_two_factor( Request $request ): array {
		...
		$this->auth_service->disable_two_factor( $request, $current_password );
	...
	public function regenerate_backup_codes( Request $request ): array {
		...
		return $this->success_response(
			array(
				'backupCodes' => $this->auth_service->regenerate_backup_codes(
					$request,
					$current_password,
				),
			),
	...
	public function download_backup_codes( Request $request ): array {
		...
		$codes            = $this->auth_service->get_backup_codes(
			$request,
			$current_password,
		);
		$body             = implode(
	...
	public function revoke_session( Request $request ): array {
		$revoked = $this->auth_service->revoke_session(
			$request,
			$this->route_param( $request, 'id' ),
		);
	...
	public function revoke_other_sessions( Request $request ): array {
		$revoked_count = $this->auth_service->revoke_other_sessions( $request );
