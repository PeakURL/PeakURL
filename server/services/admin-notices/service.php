...
use PeakURL\Services\AdminNotices\Registry;
...
class AdminNotices {
	 ...
	 *
	 * @param Registry             $registry Notice registry.
	 * @param array<string, mixed> $context  Notice context.
	 * @return void
	 * @since 1.0.3
	 */
	...
	private function add_update_notice(
		...
		$message = '' !== $latest_version && '' !== $current_version
			? sprintf(
				...
				__( 'You are running PeakURL %1$s. View the <button type="button" class="underline hover:opacity-100 js-update-details-trigger">release notes</button> and install %2$s from the Updates page.', 'peakurl' ),
				$current_version,
				$latest_version,
			)
			: __( 'View the <button type="button" class="underline hover:opacity-100 js-update-details-trigger">release notes</button> and install the latest release from the Updates page.', 'peakurl' );
