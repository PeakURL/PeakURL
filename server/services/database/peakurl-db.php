...
class PeakURL_DB {
	...
	public function in_placeholders(
		...
		return Sql::in_placeholders( $values, $prefix );
	}
	...
	public function get_row_by(
		...
		$where_clause = Sql::where_equals( $where, 'where' );

		return $this->get_row(
			'SELECT ' . Sql::select_columns( $columns ) .
	...
	public function get_results_by(
		...
		$sql    = 'SELECT ' . Sql::select_columns( $columns ) .
			' FROM ' . $this->table_identifier( $table_name );
		$params = array();
	...
	public function get_col_by(
		...
		$sql    = 'SELECT ' . Sql::quote_identifier( $column_name ) .
			' FROM ' . $this->table_identifier( $table_name );
		$params = array();
	...
	public function get_var_by(
		...
		return $this->get_var(
			'SELECT ' . Sql::quote_identifier( $column_name ) .
			' FROM ' . $this->table_identifier( $table_name ) .
			' WHERE ' . $where_clause['sql'] .
			' LIMIT 1',
			$where_clause['params'],
	...
	public function upsert(
		...
		$insert_clause = Sql::insert_values( $data, 'insert' );
		$assignments   = array();

		foreach ( $update_columns as $column_name ) {
			$column_name   = Sql::sanitize_identifier(
				(string) $column_name,
				'upsert column',
			);
			$assignments[] = Sql::quote_identifier( $column_name ) .
				' = VALUES(' .
				Sql::quote_identifier( $column_name ) .
				')';
		}
	...
	public function get_col_where_in(
		...
		list( $sql, $params ) = $this->prepare_where_in_query(
			...
			$values,
			$where,
			'SELECT ' . Sql::quote_identifier( $select_column ),
		);
	...
	public function update_where_in(
		...
		$set_clause = Sql::set_values( $data, 'set' );
		$in_clause  = Sql::where_in( $column_name, $values, $column_name );
	...
	public function insert( string $table_name, array $data ): int {
		$insert_clause = Sql::insert_values(
			$data,
			'insert',
		);
	...
	public function update(
		...
		$set_clause   = Sql::set_values(
			$data,
			'set',
		);
		$where_clause = Sql::where_equals(
			$where,
			'where',
		);
