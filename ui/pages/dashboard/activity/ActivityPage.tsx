...
function getActivityPersonName(
...
function getActivityDayGroupLabel(
...
function groupActivitiesByDay(
	...
	activities.forEach((activity) => {
		const { key, label } = getActivityDayGroupLabel(
			activity.timestamp
		);
...
function ActivityPage() {
	...
	const startItem =
		totalItems > 0 ? (meta.page - 1) * meta.limit + 1 : 0;
	const endItem =
		totalItems > 0
			? Math.min(meta.page * meta.limit, totalItems)
			: 0;
	const visiblePages = getVisiblePages(meta.page, totalPages);
	const dayGroups = groupActivitiesByDay(items);
	const allEventsCount =
		allSummaryRes?.data?.meta?.totalItems ?? totalItems;
	const linkEventsCount =
		linksSummaryRes?.data?.meta?.totalItems ?? 0;
	const userEventsCount =
		usersSummaryRes?.data?.meta?.totalItems ?? 0;
						...
						{dayGroups.length > 0 ? (
							...
							{dayGroups.map((group) => {
								...
								const selectedGroupCount =
									groupSelectableIds.filter((activityId) =>
								...
								const isGroupIndeterminate =
									selectedGroupCount > 0 && !isGroupSelected;
